...
// Custom hook to fetch multiple users by their IDs from the users collection
const useUsers = (userIds: string[]) => {
  const { firestore } = useFirebase();
  const [users, setUsers] = useState<Record<string, User>>({});
  const [isLoading, setIsLoading] = useState(false);

  // Stable reference for userIds
  ...
  useEffect(() => {
    ...
    const fetchUsers = async () => {
      ...
      const userPromises = userIds.map(async (name) => {
        try {
          ...
          console.error(`Error fetching user document for ID ${name}:`, error);
        }
        return null;
      });
...
export default function LandlordDashboard() {
  ...
  const tenantIds = useMemo(() =>
    Array.from(new Set(landlordLeases?.map(l => l.tenantId).filter(Boolean) || []))
  , [landlordLeases]);

  // Fetch all tenant user documents from users collection
  ...
  // Fetch viewing requests for these properties
            ...
            <TableBody>
              {landlordPayments && landlordPayments.length > 0 ? (
                landlordPayments.slice(0, 5).map((payment) => {
                  ...
                  const tenantName = tenantId && tenants[tenantId]?.name ? tenants[tenantId].name : 'N/A';
